...
p <- d |> 
    ...
    labs(x = "Orkuframleiðsla á mann",
         ...
         subtitle = str_c("Heilar línur sýna mismunandu hlutföll orku- / landsframleiðslu",
                          "\n",
...
```{r}
start <- d |> filter(year == 2018, energy_per_capita > 0) |> pull(energy_per_capita) |> min(na.rm = T)
end <- d |> filter(year == 2018, energy_per_capita > 0) |> pull(energy_per_capita) |> max(na.rm = T)

min_gdp <- d |> filter(year == 2018) |> pull(gdp_per_capita) |> min(na.rm = T)
max_gdp <- d |> filter(year == 2018) |> pull(gdp_per_capita) |> max(na.rm = T)
...
d |> 
    filter(year == 2018) |> 
    select(gdp_per_capita, energy_per_capita, country) |> 
    drop_na() |> 
...
```{r}
m <- lm(log(energy_per_gdp) ~ log(energy_per_capita), data = d |> filter(year == 2018))
d |> 
    filter(year == 2018) |> 
    select(country, energy_per_capita, energy_per_gdp) |> 
    drop_na() |> 
    mutate(pred = predict(m) |> exp()) |> 
    ggplot(aes(energy_per_capita, energy_per_gdp,
               colour = (country == "Iceland"))) +
    ...
    theme(legend.position = "none") +
    labs(x = "Orkuframleiðsla á mann",
         y = "Orkuframleiðsla deilt með landsframleiðslu")
```

```{r}
d |> 
    filter(year == 2018) |> 
    top_n(10, renewables_share_energy) |> 
    arrange(desc(renewables_share_energy))
```

# Samanburður við lönd með mest af endurnýjanlegri orku

```{r}
d |> 
    filter(country %in% c("Iceland", "Norway", "New Zealand", "Lithuania", "Luxembourg"),
           year >= 1980) |> 
    select(year, country, energy_per_gdp) |> 
    drop_na() |> 
    ggplot(aes(year, energy_per_gdp, group = country, col = country)) +
    geom_line()
```
...
d |> 
    ...
    drop_na() |> 
    ggplot(aes(year, energy_per_capita, group = country, col = country)) +
    geom_line()
```
...
d |> 
    ...
    drop_na() |> 
    ggplot(aes(year, gdp_per_capita, group = country, col = country)) +
    geom_line()
```
